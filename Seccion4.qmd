...
## Sección. Deportes y Esparcimiento
...
```{r, message=FALSE, warning=FALSE, echo=FALSE, fig.width=12, fig.height=7}
library(ggplot2)
library(ggwordcloud)
library(dplyr)
library(stringr)
...
# 1. Tomar variable y limpiar
palabras <- datos$otras_lib |>
  na.omit() |>
  as.character() |>
  str_to_lower() |>
  str_squish()

# 2. Separar respuestas múltiples por coma, punto y coma o slash
palabras <- str_split(palabras, pattern = ",|;|/") |> 
  unlist()
...
# 4. Eliminar valores no válidos
palabras <- palabras[
  !palabras %in% c(
    "", "0", "n/a", "na", "no aplica", "no", 
    "ninguno", "ninguna", "otros", "otras", 
    "otro", "otra", "etc", "no fue otra"
  )
]

# 5. Eliminar frases vacías o demasiado cortas
...
ggplot(df_palabras, aes(label = word, size = freq, color = word)) +
  ...
  scale_color_manual(values = sample(colors(), nrow(df_palabras))) +
  theme_minimal() +
  theme(
...
ggplot(df_palabras, aes(label = word, size = freq, color = word)) +
  ...
  scale_size_area(max_size = 65) +   # 🔴 antes 120 → distorsiona en GitHub
...
# 1. Extraer y limpiar texto
palabras <- datos$otros_dep |>
  na.omit() |>
  as.character() |>
  str_to_lower() |>
  str_squish()

# 2. Separar respuestas múltiples
palabras <- str_split(palabras, pattern = ",|;|/") |>
  unlist()
...
# 4. Eliminar palabras muy cortas
palabras <- palabras[nchar(palabras) > 2]

# 5. Crear tabla de frecuencia
df_palabras <- data.frame(word = palabras) |>
  count(word, name = "freq") |>
  arrange(desc(freq)) |>
  slice_head(n = 40)
...
datos_plot <- datos %>%
  filter(!is.na(campus), !is.na(salud_ment),
         campus %in% c("Bucaramanga", "Cucuta", "Valledupar")) %>%
...
datos_final <- datos_final %>%
  mutate(salud_ment = reorder(salud_ment, porcentaje))
...
fig <- plot_ly(
  ...
  hovertemplate = paste(
    "<b>%{y}</b><br>",
    "Porcentaje: %{x}%<br>",
    "Frecuencia: %{customdata}<extra></extra>"
  ),
  customdata = ~n,
  marker = list(color = "#a6cee3"),
  transforms = list(
...
# 1. Preparar los datos
datos_plot <- datos %>%
  filter(!is.na(campus), !is.na(fam_origen),
         campus %in% c("Bucaramanga", "Cucuta", "Valledupar")) %>%
...
# 5. Ordenar categorías (solo en "Todos")
orden_niveles <- datos_final %>%
  filter(campus == "Todos") %>%
  arrange(desc(porcentaje)) %>%
  pull(fam_origen)

datos_final <- datos_final %>%
  mutate(fam_origen = factor(fam_origen, levels = orden_niveles))
...
# 1. Preparar los datos
datos_plot <- datos %>%
  filter(!is.na(campus), !is.na(fam_formada),
         campus %in% c("Bucaramanga", "Cucuta", "Valledupar")) %>%
...
datos_final <- datos_final %>%
  mutate(fam_formada = factor(fam_formada, levels = orden_niveles))
