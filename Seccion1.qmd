...
::: {.callout-note title="Descripción para todos los campus"}
En relación con el dominio de lenguas extranjeras, el **18%** de los estudiantes manifiesta contar con esta competencia (**Sí**). Por su parte, en cuanto a la certificación formal en lengua extranjera, el **8.8%** de los estudiantes indica disponer de este respaldo (**Sí**).
:::
...
datos_clean <- datos %>%
  mutate(
    ...
    leng_ext = case_when(
      leng_ext %in% c("Si", "Sí") ~ "Sí",
      leng_ext == "No" ~ "No",
      TRUE ~ NA_character_
    ),

    cert_leng = case_when(
      cert_leng %in% c("Si", "Sí") ~ "Sí",
      cert_leng %in% c("No", "0", "") ~ "No",
      TRUE ~ NA_character_
    )
  ) %>%
  filter(!is.na(leng_ext), !is.na(cert_leng))
...
# =========================
# 3. Calcular porcentajes correctamente
# =========================
tabla <- datos_long %>%
...
# 4. Orden correcto
# =========================
tabla$respuesta <- factor(tabla$respuesta, levels = c("Sí", "No"))

# =========================
# 5. Gráfico final (CORRECTO)
# =========================
plot_ly(
  data = tabla,
  x = ~variable,
